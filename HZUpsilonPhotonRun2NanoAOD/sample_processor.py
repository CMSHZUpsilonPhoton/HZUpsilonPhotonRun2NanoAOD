...
class SampleProcessor:
    def __init__(
        self, events, dataset, year, data_or_mc, output, weighted_sum_of_events
    ):
        self.events = events
        self.dataset = dataset
        self.year = year
        self.data_or_mc = data_or_mc
        self.output = output
        self.weighted_sum_of_events = weighted_sum_of_events
    ...
    def call(self):
        ...
        # Event weight holder
        weights = analysis_tools.Weights(size=len(self.events), storeIndividual=True)
        ...
        self.output["cutflow"].histogram.fill(
            **cutflow_filling_parameters(self.events, filters_masks, weights)
        )
