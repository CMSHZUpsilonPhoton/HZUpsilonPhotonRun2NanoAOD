...
def save_kinematical_information(
    boson_combinations, dataset, year, prefix, weights, selected_events_filter
):
    ...
    with uproot.recreate(output_filename) as f:
        f["Events"] = {
                ...
                "delta_eta_upsilon_photon": ak.flatten(
                    np.absolute(upsilons.eta - photons.eta)
                ),
                "delta_phi_upsilon_photon": ak.flatten(
                    np.absolute(upsilons.delta_phi(photons))
                ),
