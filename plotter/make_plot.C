...
//	const char *selection[2] = {"preselected","selected"};
	...
	for(int j=0;j<n_histos;j++){
		...
		for(int i=0; i<tree->GetEntries(); i++){
			tree->GetEntry(i);
			if(j!=1 && j!=5) h1_data->Fill(var,var_w);
                        if(j==1 && !((var_d<96 && var_d>86)||(var_d<135 && var_d>115))) h1_data->Fill(var_d,var_w);//blind signal ragion in boson mass plot
                        if(j==5) h1_data->Fill(var_d,var_w);
		}
		...
		for(int i=0; i<tree1->GetEntries(); i++){
			tree1->GetEntry(i);
			if(j!=1 && j!=5) h1_MC1->Fill(var1,var1_w);
                        if(j==1||j==5)h1_MC1->Fill(var1_d,var1_w);
		}
		...
		for(int i=0; i<tree2->GetEntries(); i++){
			tree2->GetEntry(i);
			if(j!=1 && j!=5) h1_MC2->Fill(var2,var2_w);
			if(j==1||j==5) h1_MC2->Fill(var2_d,var2_w);
		}
		...
		for(int i=0; i<tree3->GetEntries(); i++){
			tree3->GetEntry(i);
			if(j!=1 && j!=5) h1_MC3->Fill(var3,var3_w);
                        if(j==1||j==5) h1_MC3->Fill(var3_d,var3_w);
		}
		...
		for(int i=0; i<tree4->GetEntries(); i++){
			tree4->GetEntry(i);
			if(j!=1 && j!=5) h1_MC4->Fill(var4,var4_w);
                        if(j==1||j==5) h1_MC4->Fill(var4_d,var4_w);
		}
		...
		for(int i=0; i<tree5->GetEntries(); i++){
			tree5->GetEntry(i);
			if(j!=1 && j!=5) h1_MC5->Fill(var5,var5_w);
                        if(j==1||j==5) h1_MC5->Fill(var5_d,var5_w);
		}
		...
		for(int i=0; i<tree6->GetEntries(); i++){
			tree6->GetEntry(i);
			if(j!=1 && j!=5) h1_MC6->Fill(var6,var6_w);
                        if(j==1||j==5) h1_MC6->Fill(var6_d,var6_w);
		}
		...
		Double_t var7_d;
                ...
                for(int i=0; i<tree7->GetEntries(); i++){
                        tree7->GetEntry(i);
                        if(j!=1 && j!=5) h1_MC7->Fill(var7,var7_w);
                        if(j==1||j==5) h1_MC7->Fill(var7_d,var7_w);
                }
		...
		Double_t var8_d;
                ...
                for(int i=0; i<tree8->GetEntries(); i++){
                        tree8->GetEntry(i);
                        if(j!=1 && j!=5) h1_MC8->Fill(var8,var8_w);
                        if(j==1||j==5) h1_MC8->Fill(var8_d,var8_w);
                }
